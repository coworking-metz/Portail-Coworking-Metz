...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	...
	<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button button btn btn-sm font-weight-bold text-uppercase ltr-sp-15" aria-label="<?php esc_attr_e( 'Add "', 'ave' ); ?><?php echo $product->get_name(); ?><?php esc_attr_e( '" to your cart', 'ave' ); ?>" data-product_id="<?php echo esc_attr( $product->get_id() ); ?>" data-product_sku="<?php echo esc_attr( $product->get_sku() ); ?>"><span><?php echo esc_html( $product->single_add_to_cart_text() ); ?></span></button>
