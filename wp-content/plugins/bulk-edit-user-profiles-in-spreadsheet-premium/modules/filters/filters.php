	...
	class WP_Sheet_Editor_Filters {
		...
		function _sanitize_filters($filters) {
			...
			foreach ($filters as $filter_key => $filter) {
				...
				if (in_array($filter_key, array('search_name', 'date_from', 'date_to'))) {
				...
				} elseif ($filter_key === 'post_author' && current_user_can('edit_others_posts')) {
					...
					foreach ($filter as $meta_query) {
						...
						if (empty($meta_query['key']) || empty($meta_query['compare']) || empty($meta_query['source']) || !isset($meta_query['value'])) {
							continue;
						}
						$out[$filter_key][] = array(
					...
					<form action="<?php echo esc_url(admin_url()); ?>" method="GET" id="be-filters" >
						...
						<ul class="unstyled-list basic-filters">
							<?php if (isset($filters['keyword'])) { ?>
								<li>
									<label><?php echo wp_kses_post($filters['keyword']['label']); ?> <?php if (!empty($filters['keyword']['description'])) { ?><a href="#" class="tipso" data-tipso="<?php echo esc_attr($filters['keyword']['description']); ?>">( ? )</a><?php } ?></label><input type="text" name="keyword" />
								</li>
							<?php } ?>
							<?php if (isset($filters['post_status'])) { ?>
								<li>
									<label><?php echo wp_kses_post($filters['post_status']['label']); ?>  <?php if (!empty($filters['post_status']['description'])) { ?><a href="#" class="tipso" data-tipso="<?php echo esc_attr($filters['post_status']['description']); ?>">( ? )</a><?php } ?></label>
							...
							<?php if (isset($filters['post_author']) && post_type_supports($current_post_type, 'author')) { ?>
								<li>
									<label><?php echo wp_kses_post($filters['post_author']['label']); ?>  <?php if (!empty($filters['post_author']['description'])) { ?><a href="#" class="tipso" data-tipso="<?php echo esc_attr($filters['post_author']['description']); ?>">( ? )</a><?php } ?></label>
