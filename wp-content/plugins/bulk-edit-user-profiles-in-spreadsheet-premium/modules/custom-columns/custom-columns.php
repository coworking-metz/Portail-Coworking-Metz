...
if (!class_exists('WP_Sheet_Editor_Custom_Columns')) {
	...
	class WP_Sheet_Editor_Custom_Columns {
		...
		function register_columns_automatically($editor) {
			...
			if (function_exists('WPSE_Profiler')) {
				WPSE_Profiler()->record("Before registering serialized field " . __FUNCTION__);
			}
			...
			if (function_exists('WPSE_Profiler')) {
				WPSE_Profiler()->record("End " . __FUNCTION__);
			}
		}
		...
		function detect_column_type($meta_key, $editor, $post_types_for_sample_values = array()) {
			$values = array();
		...
		function rename_meta_key() {
			...
			if (empty($_REQUEST['post_type']) || empty($_REQUEST['old_column_key']) || empty($_REQUEST['new_column_key']) || $_REQUEST['old_column_key'] === $_REQUEST['new_column_key']) {
			...
			$post_type = VGSE()->helpers->sanitize_table_key($_REQUEST['post_type']);
			$old_column_key = sanitize_text_field($_REQUEST['old_column_key']);
			$new_column_key = sanitize_text_field($_REQUEST['new_column_key']);
		...
		function save_columns() {
			...
			if (empty($_REQUEST['columns'])) {
				...
				$columns = array();
				foreach ($_REQUEST['columns'] as $index => $dirty_column_args) {
