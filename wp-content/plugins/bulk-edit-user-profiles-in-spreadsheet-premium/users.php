	...
	class WP_Sheet_Editor_Users {
		...
		function add_settings_page_options__premium_only($sections) {
			$sections['users'] = array(
				'icon' => 'el-icon-cogs',
				'title' => __('Users sheet', VGSE()->textname),
				'fields' => array(
					array(
						'id' => 'users_hide_administrators',
						'type' => 'switch',
						'title' => __('Hide administrators on the spreadsheet?', VGSE()->textname),
						'desc' => __('We will not show administrator users in the spreadsheet, so no one can see or edit them, even administrators wont be able to see other administrators.', VGSE()->textname),
						'default' => false,
					),
					array(
						'id' => 'users_allowed_roles',
						'type' => 'text',
						'title' => __('Which user roles will be displayed and edited in the spreadsheet?', VGSE()->textname),
						'desc' => __('Enter the list of role keys separated by commas. For example, if you only want to edit and see customers on the spreadsheet', VGSE()->textname),
					),
				)
			);
			return $sections;
		...
		function notify_free_limitations_above_table($post_type) {
			...
			printf(__('. <b>Lite version</b> listing "subscriber" users. <b>Go pro:</b> edit all the roles (%s), custom fields, export, import, and more', VGSE()->textname), esc_html(str_replace(', Subscriber', '', implode(', ', VGSE_Users_Helpers_Obj()->get_all_the_roles()))));
			echo '</span>';
		}
