...
abstract Class ModuleElementorAbstract extends Widget_Base {
	...
	private function get_class( $param ) {
		...
		return "\EPIC\Module\\" . $mod[1] . "\\" . $mod[1] . "_" . $mod[2] . "_" . $param;
	}

	private function get_option_instance() {
		if ( ! $this->option_instance ) {
			$option_class          = apply_filters( 'epic_module_elementor_get_option_class', $this->get_class( 'Option' ) );
	...
	private function get_view_instance() {
		if ( ! $this->view_instance ) {
			$view_class          = apply_filters( 'epic_module_elementor_get_view_class', $this->get_class( 'View' ) );
	...
	public function get_icon() {
		return epic_get_shortcode_name_from_option( get_class( $this->get_option_instance() ) );
	}
	...
	private function build_option( $options ) {
		...
		foreach ( $group_options as $group => $options ) {
			...
			if ( $group === 'style' ) {
				$section = array(
					'label' => esc_html__( 'Style', 'epic-ne' ),
					'tab'   => Controls_Manager::TAB_STYLE
				);
			} else if ( $group === 'setting' ) {
				$section = array(
					'label' => esc_html__( 'Setting', 'epic-ne' ),
	...
	private function parse_control_option( $option ) {
		$option['heading'] = isset( $option['heading'] ) ? $option['heading'] : '';
	...
	protected function render() {
		...
		echo jeg_sanitize_output( $this->get_view_instance()->build_module( $settings ) );
	}
