...
abstract Class ModuleViewAbstract
{
    ...
    public function get_vc_class_name()
    {
        ...
        if(isset($this->attribute['boxed']) && $this->attribute['boxed']) $class_name .= ' jeg_pb_boxed';
	    if(isset($this->attribute['boxed_shadow']) && $this->attribute['boxed_shadow']) $class_name .= ' jeg_pb_boxed_shadow';
    ...
    public function render_uncompatible()
    {
        ...
        $text = wp_kses(sprintf(__('This module works best for column <strong>%s</strong> ( current column width <strong>%s</strong> ). This warning will only show if you login as Admin.', 'epic-ne'), implode(', ', $compatible), $column), wp_kses_allowed_html());
        $element =
            ...
            foreach ( $ads_images as $key => $ads_image )
            {
	            if ( filter_var($ads_images, FILTER_VALIDATE_URL) === FALSE )
	            {
		            if ( isset( $ads_image['url'] ) && ! empty( $ads_image['url'] ) )
		            {
                        ...
                        $ads_images = wp_get_attachment_image_src($ads_image, 'full');
			            $ads_images[$key]  = isset( $ads_images[0] ) ? $ads_images[0] : '';
                    }
	            }
            }
