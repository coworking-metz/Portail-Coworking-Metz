...
Class Element_Header_View extends ModuleViewAbstract
{
    public function render_module($attr, $column_class)
    {
	    ...
	    $attr['first_title']  = empty( $attr['first_title'] ) ? esc_html__( 'First Title', 'epic-ne' ) : $attr['first_title'];

        $heading_title  = $attr['first_title'] . $subtitle;

        if(!empty($heading_title))
        {
            $heading_icon   = isset( $attr['header_icon'] ) ? epic_header_icon($attr['header_icon']) : '';
        ...
        $style_output   = epic_header_styling($attr, $this->unique_id);
        ...
        if(empty($heading_title)) {
            ...
            $output =
                "<div {$this->element_id($attr)} class=\"jeg_block_heading {$header_class} jeg_align{$attr['header_align']} {$this->unique_id} {$this->get_vc_class_name()} {$this->color_scheme()} {$attr['el_class']}\">
                    {$heading_title}
                    {$style}
                </div>";
        }
