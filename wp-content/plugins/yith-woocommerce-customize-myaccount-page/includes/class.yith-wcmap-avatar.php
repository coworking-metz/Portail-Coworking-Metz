...
if ( ! class_exists( 'YITH_WCMAP_Avatar' ) ) {
	...
	class YITH_WCMAP_Avatar {
		...
		protected function upload_avatar() {
			...
			if ( empty( $_FILES['ywcmap_custom_avatar'] ) || apply_filters( 'ywcmap_prevent_upload_user_avatar', false, $_FILES['ywcmap_custom_avatar'] ) ) {
				wp_send_json_error();
			}
		...
		protected function delete_avatar( $avatar_key ) {
			...
			$key        = array_search( $avatar, $avatar_ids );
			if ( false !== $key ) {
...
new YITH_WCMAP_Avatar(); // phpcs:ignore
