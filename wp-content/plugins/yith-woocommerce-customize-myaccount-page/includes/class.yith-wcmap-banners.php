...
if ( ! class_exists( 'YITH_WCMAP_Banners' ) ) {
	...
	class YITH_WCMAP_Banners {
					...
					if ( 'yes' === $banner['show_counter'] && $customer ) {
						switch ( $banner['counter_type'] ) {
							...
							case 'downloads':
								...
								break;
							default:
								$banner['counter'] = apply_filters( 'yith_wcmap_banner_' . str_replace( '-', '_', $banner['counter_type'] ) . '_counter_value', 0, $customer );
								break;
						}
					}
		...
		public function admin_init() {
			...
			if ( ! class_exists( 'YITH_WCMAP_Admin' ) || ! isset( $_REQUEST['page'] )
				 || sanitize_text_field( $_REQUEST['page'] ) !== YITH_WCMAP_Admin::PANEL_PAGE ) {
				return;
			}
		...
		public function banner_shortcode( $atts ) {
			...
			$banners_key      = array_flip( array_unique( explode( ',', $atts['ids'] ) ) );
		...
		public function banners_modal() {
			...
			include YITH_WCMAP_TEMPLATE_PATH . '/admin/banners-modal.php';
		}
	}
}
