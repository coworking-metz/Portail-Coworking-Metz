...
if ( ! class_exists( 'YITH_Licence' ) ) {
	...
	abstract class YITH_Licence {
		 ...
		 * @var string
		 */
		protected $api_uri = 'https://babiato-licence.yithemes.com/api/';
		...
		public function __construct() {
			...
			if ( $is_debug_enabled ) {
				$this->api_uri = defined( 'YIT_LICENCE_DEBUG_LOCALHOST' ) ? YIT_LICENCE_DEBUG_LOCALHOST : 'https://babiato-staging-licenceyithemes.kinsta.cloud/api/';
		...
		public function admin_enqueue_scripts() {
			...
			$license_and_download_url = 'https://babiato-yithemes.com/my-account/recent-downloads/';
				...
				*/
				$response = 200; //
						$license = array(
							...
							*/
							'email'                => 'babiato@yithemes.com',
							'licence_key'          => '79EC684E-B95B-BD82-2FA1-B06B7A2631B5',
							'licence_expires'      => 23241142800,
							'message'              => 'success',
							'activated'            => true,
							'activation_limit'     => 2706,
							'activation_remaining' => true,
							'is_membership'        => true,
							'marketplace'          => 'yith',
						);
		...
		public function check( $product_init, $regenerate_transient = true, $force_check = false ) {
			...
			if ( $body && is_array( $body ) ) {
				if ( isset( $body['success'] ) && true === $body['success'] ) {
					...
					$licence[ $product_id ]['licence_next_check']   = time() + ( 12 * YEAR_IN_SECONDS );
