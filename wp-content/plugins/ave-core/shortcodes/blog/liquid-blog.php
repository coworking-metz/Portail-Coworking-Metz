		...
		$general = array(
			array(
				...
				'value'      => array(
					...
					array(
						'label' => esc_html__( 'Podcast', 'ave-core' ),
						'value' => 'podcast',
						'image' => $url . 'podcast.jpg'
					),
	...
	protected function entry_thumbnail( $size = 'liquid-thumbnail', $attr = '', $background = false ) {
		...
		if( has_post_thumbnail() ) {
			printf( '<figure class="liquid-lp-media %s" %s %s>', $figure_classnames, $figure_background, $this->enable_parallax() );
				...
				echo '</a>';
				if ( ( 'podcast' === $this->atts['style'] || 'grid' === $this->atts['style'] ) && (get_post_format() === 'audio' || get_post_format() === 'video') ){
					?>
					<div class="lqd-lp-post-type-button pos-abs">
						<a href="<?php the_permalink(); ?>" class="btn btn-md btn-naked btn-icon-solid btn-icon-block btn-icon-top btn-icon-circle btn-icon-lg">
							<span>
								<span class="btn-icon">
