...
class YITH_WCCOS_Frontend_Manager_Integration {
	...
	public function get_inline_css() {
		...
		foreach ( $status_ids as $status_id ) {
			...
			$icon_data      = isset( $icon['icon'] ) ? $icon['icon'] : 'FontAwesome:genderless';
			$is_custom_icon = isset( $icon['select'] ) && $icon['select'] !== 'none';
			...
			$icon_key = array_key_exists( $icon_font, $icons ) ? array_search( $icon_name, $icons[ $icon_font ] ) : '';
			...
			if ( 'text' === $graphicstyle ) {
				...
				if ( ! $is_custom_icon && in_array( $name, $wc_status ) ) {
