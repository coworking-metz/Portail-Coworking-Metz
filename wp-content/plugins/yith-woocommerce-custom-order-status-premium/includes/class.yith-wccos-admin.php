...
if ( ! class_exists( 'YITH_WCCOS_Admin' ) ) {
	...
	class YITH_WCCOS_Admin {
		...
		function add_submit_to_order_admin_actions( $actions, $the_order ) {
			...
			foreach ( $status_posts as $sp ) {
				...
				if ( $meta['slug'] == 'completed' ) {
					$actions['complete'] = array(
						'url'    => wp_nonce_url( admin_url( 'admin-ajax.php?action=woocommerce_mark_order_status&status=' . $meta['slug'] . '&order_id=' . $order_id ), 'woocommerce-mark-order-status' ),
						'name'   => $meta['label'],
						'action' => 'complete',
					);
				} else {
		...
		public function action_links( $links ) {
			return yith_add_action_links( $links, $this->_panel_page, defined( 'YITH_WCCOS_PREMIUM' ), YITH_WCCOS_SLUG );
		}
