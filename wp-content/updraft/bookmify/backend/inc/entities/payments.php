									...
									<div class="info_holder">
										<div class="info_in">
											...
											<span class="payment_customer">
												<span class="p_service">'.Helper::titleDecryption($payments->data[$i]->serviceTitle).'</span>
												<span>'.$payments->data[$i]->customerFirstName.' '.$payments->data[$i]->customerLastName.'</span>
											</span>
											<span class="payment_service"><span>'.Helper::titleDecryption($payments->data[$i]->serviceTitle).'</span></span>
	...
	public function ajaxQueryOpenPaymentDetails(){
		...
		if (!empty($_POST['bookmify_data'])) {
			...
			foreach($payments as $payment){
				...
				$pServiceTitle		= Helper::titleDecryption($payment->serviceTitle);
				// ------------------------------------------------------------------------------------------------
				...
				for($i=0; $i<count($quantityExtras); $i++){
					if(is_int($quantityExtras[$i]) && is_int($priceExtrasIn[$i])){
